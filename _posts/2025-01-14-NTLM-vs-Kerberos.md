...
(4) Сервер отправляет следующие три элемента контроллеру домена:<br>
   - Имя пользователя
   - Вызов отправлен клиенту
   - Ответ, полученный от клиента<p>
...
(6) Затем сервер отправляет соответствующий ответ обратно клиенту.<p>
...
(2) Сервер аутентификации проверит, существует ли пользователь в базе данных KDC.<br>
...
(6) Сервер расшифровывает запрос, и если он подлинный, он обеспечивает доступ к службе.<p>
